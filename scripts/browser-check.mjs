...
await page.click('button:text-is("Markierung bestätigen")');
await page.waitForSelector("text=Zur Löschung vorgemerkt", { timeout: 15000 });
record("Löschmarkierung gesetzt ohne Datenverlust", (await page.locator("canvas").count()) > 0);
await page.click('button:text-is("Markierung aufheben")');
...
for (const name of TEST_NAMES) {
  ...
  const markButton = testRow.locator('button:text-is("Zur Löschung markieren")');
